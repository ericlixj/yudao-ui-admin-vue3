<template>
  <Dialog v-model="dialogVisible" title="请购单明细导入" width="400">
    <el-upload
      ref="uploadRef"
      v-model:file-list="fileList"
      :action="importUrl + '?updateSupport=' + updateSupport + '&reqPurchaseCode=' + props.reqPurchaseCode"
      :auto-upload="false"
...
defineOptions({ name: 'PurchaseReqOrderItemsImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/cacerp/purchase/purchase-req-order/importItems'
...
const importTemplate = async () => {
  const res = await PurchaseReqOrderApi.importPurchaseReqOrderItemsTemplate()
  download.excel(res, '请购单明细导入模版.xls')
}
