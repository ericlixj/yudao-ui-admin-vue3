  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="税务编号" prop="taxNo">
        <el-input v-model="formData.taxNo" placeholder="请输入税务编号" />
      </el-form-item>
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await CacErpSupplierApi.getCacErpSupplier(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as CacErpSupplierVO
    if (formType.value === 'create') {
      await CacErpSupplierApi.createCacErpSupplier(data)
      message.success(t('common.createSuccess'))
    } else {
      await CacErpSupplierApi.updateCacErpSupplier(data)
